...
for i in range(len(products)):
    for j in range(i + 1, len(products)):
        ...
        best_lag = np.argmax(abs_pearson_for_max_lag) + 1
        best_lags.append(best_lag)
        pearson_correlations.append(pearson_correlations_for_max_lag[best_lag - 1])

# 输出结果
for i in range(len(products)):
    for j in range(i + 1, len(products)):
        k = i * (len(products) - i - 1) // 2 + j - i - 1
...
for i in range(len(products)):
    for j in range(i + 1, len(products)):
        ...
        corr_mat[j, i] = pearson_correlations[k]
...
plt.savefig('/Users/zhangyichi/Desktop/商品价格相关性热图/商品价格相关性热图.png', dpi=300, bbox_inches='tight')
plt.show()
...
if not os.path.exists('/Users/zhangyichi/Desktop/商品价格相关性热图'):
    os.makedirs('/Users/zhangyichi/Desktop/商品价格相关性热图')
plt.savefig('/Users/zhangyichi/Desktop/商品价格相关性热图/商品价格滞后期热图.png', dpi=300, bbox_inches='tight')
plt.show()
